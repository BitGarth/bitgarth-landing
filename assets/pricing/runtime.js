...
export async function refreshPricing({ fetchFn, getGrid, getNote = () => null }) {
  try {
    ...
    const { cardsHTML, summaryHTML } = renderPricing(snapshot);
    if (grid.innerHTML.trim() !== cardsHTML.trim()) {
      grid.innerHTML = cardsHTML;
    }
    const note = getNote();
    if (note && summaryHTML && note.innerHTML.trim() !== summaryHTML.trim()) {
...
if (typeof document !== 'undefined') {
  const boot = () => refreshPricing({
    ...
    getNote: () => document.getElementById('pricing-note'),
  });
